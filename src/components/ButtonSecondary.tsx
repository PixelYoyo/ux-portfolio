...
export default function ButtonSecondary({ label, href, className = '' }: Props) {
  const { displayed, active, onMouseEnter, onMouseLeave } = useTypewriter(label);

  const cls = `border-[3px] border-border-brand flex items-center justify-center px-4xl py-3xl cursor-pointer ${className}`;
